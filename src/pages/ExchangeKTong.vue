<template>
  <div class="exchange">
    <KTop :title="'兑换通贝'">
      <p class="slot" @click="toRecord('2')">兑换记录</p>
    </KTop>
    <div class="bgImg">
      <div class="bg-l">
      ...
      <div class="bg-r">
        <p class="money">{{clientInfo.unlock_kci_balance || 0}}</p>
        <p class="text">GSC</p>
      </div>
    </div>
    ...
    <div class="content">
      <div class="change">
        ...
        <div class="change-text">
          <span>GSC</span>
          <input v-model="balance" type="tel" placeholder="请输入兑换的GSC"/>
        </div>
        <p>转换通贝</p>
  ...
  export default {
    ...
    methods: {
      ...
      openPay() {
        ...
        if (Number(this.clientInfo.unlock_kci_balance) < Number(this.balance)) {
          this.$toast('账户余额不足')
          return
        }
        ...
        if (Number(this.balance) > 2000) {
          this.$toast('GSC兑换通贝限额2000')
          return
        }
