<template>
  <div class="Publish">
    ...
    >
      ...
      <div class="number">
        <p class="text">GSC{{text}}</p>
        <input v-model="blance" type="tel" placeholder="数额（100的整数倍）">
      </div>
  ...
  export default {
    ...
    async created() {
      ...
      if (this.$route.query.type === 'buy') {
        await api.getBuyPremium().then(res => {
      ...
      if (this.$route.query.type === 'sale') {
        await api.getSalePremium().then(res => {
          this.allNumber = res.data
        })
        this.text = '出售'
        this.orderText = '收款'
    ...
    methods: {
      ...
      openPay() {
        ...
        if (Number(this.blance) < 100 || Number(this.blance) % 100 !== 0) {
      ...
      goNext(where) {
        this.$router.push({path: `/${where}`, query: {'type': this.$route.query.type, 'current': '2'}})
      },
      more() {
      ...
      async createOrder() {
        ...
        if (this.$route.query.type === 'buy') {
          await api.addGscBuy({
            'unlock_kci_balance': this.blance,
